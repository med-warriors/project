...
if (Meteor.isServer) {
  describe('SupplyCollection', function testSuite() {
      ...
      fc.assert(
        fc.property(fc.lorem(2), fc.integer(1, 10), fc.lorem(1), fc.lorem(2),
          (name, quantity, owner, location) => {
            const docID = SupplySourceRecord.define({
              name,
              quantity,
              owner,
              location,
            ...
            SupplySourceRecord.removeIt(docID);
    ...
    it('Can define duplicates', function test2() {
      ...
      const docID1 = SupplySourceRecord.define({ name, quantity, location, owner });
      const docID2 = SupplySourceRecord.define({ name, quantity, location, owner });
      ...
      fc.assert(
        fc.property(fc.lorem(2), fc.integer(10), fc.lorem(2),
          (newName, newQuantity, newLocation) => {
            SupplySourceRecord.update(docID, {
              name: newName,
              quantity: newQuantity,
              location: newLocation,
            });
            const supply = SupplySourceRecord.findDoc(docID);
    ...
    it('Can dumpOne, removeIt, and restoreOne', function test4() {
      const origDoc = SupplySourceRecord.findOne({});
      let docID = origDoc._id;
      const dumpObject = SupplySourceRecord.dumpOne(docID);
      SupplySourceRecord.removeIt(docID);
      expect(SupplySourceRecord.isDefined(docID)).to.be.false;
      docID = SupplySourceRecord.restoreOne(dumpObject);
      expect(SupplySourceRecord.isDefined(docID)).to.be.true;
      const doc = SupplySourceRecord.findDoc(docID);
